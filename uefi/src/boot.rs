...
use crate::table::boot::{
    AllocateType, BootServices, EventNotifyFn, EventType, LoadImageSource, MemoryMap, MemoryMapKey,
    MemoryMapSize, MemoryType, OpenProtocolAttributes, OpenProtocolParams, SearchType,
    TimerTrigger, Tpl, IMAGE_HANDLE,
};
...
pub fn memory_map_size() -> MemoryMapSize {
    unsafe { boot_services().as_mut() }.memory_map_size()
}
...
/// * [`uefi::Status::INVALID_PARAMETER`]
pub fn memory_map(buffer: &mut [u8]) -> Result<MemoryMap<'_>> {
    unsafe { boot_services().as_mut() }.memory_map(buffer)
}
...
/// `SystemTable<Boot>::exit_boot_services_raw()` method, which is why it is not
/// public.
///
...
///
/// * [`uefi::Status::INVALID_PARAMETER`]
pub unsafe fn exit_boot_services(image: Handle, mmap_key: MemoryMapKey) -> Result {
    // TODO: unset image handle
    (boot_services_raw().as_mut().exit_boot_services)(image.as_ptr(), mmap_key.0).to_result()
}
