...
use uefi_raw::protocol::network::tcpv4::{
    Ipv4ModeData, Tcpv4AccessPoint, Tcpv4CompletionToken, Tcpv4ConfigData, Tcpv4ConnectionState,
    Tcpv4FragmentData, Tcpv4IoToken, Tcpv4Option, Tcpv4Packet, Tcpv4Protocol, Tcpv4ReceiveData,
    Tcpv4TransmitData,
};
...
/// use uefi::{
///     boot, print, println,
///     proto::network::tcpv4::{AccessPoint, ConfigData, Tcpv4,
///     Tcpv4ServiceBinding,
/// },
/// };
/// use uefi_raw::{
///     Boolean, protocol::network::tcpv4::Tcpv4AccessPoint,
///     protocol::network::tcpv4::Tcpv4ClientConnectionModeParams,
/// };
///
...
impl Tcpv4 {
    /// See [Tcpv4Protocol::configure].
    pub fn configure(&mut self, config: &ConfigData, options: Option<&ConfigOptions>) -> Result {
        let control_option = options.map(Tcpv4Option::from);
        let tcpv4_config_data = Tcpv4ConfigData {
            type_of_service: config.type_of_service,
            time_to_live: config.time_to_live,
            access_point: Tcpv4AccessPoint::from(&config.access_point),
    ...
    pub fn connect(&mut self) -> Result {
        unsafe {
            ...
            let mut completion_token = helpers::make_completion_token(&event);
    ...
    /// See [Tcpv4Protocol::transmit].
    pub fn transmit_vectored<const N: usize>(&mut self, data: &[&[u8]; N]) -> Result {
...
impl Tcpv4 {
    ...
    /// Tcpv4Protocol.
    const fn this(&mut self) -> *mut Tcpv4Protocol {
...
mod helpers {
    ...
    use uefi::Event;
    use uefi_raw::protocol::network::tcpv4::{
        Tcpv4CompletionToken, Tcpv4FragmentData, Tcpv4IoToken, Tcpv4Packet, Tcpv4ReceiveData,
        Tcpv4TransmitData,
    };
    ...
    impl<'a> ReceiveData<'a> {
        pub fn new(data: &'a [&mut [u8]]) -> Self {
            ...
            let header_layout = Layout::new::<Tcpv4ReceiveData>();
            let payload_layout =
            ...
            let ptr = unsafe {
                let ptr = alloc(layout).cast::<Tcpv4ReceiveData>();
                ...
                ptr::copy_nonoverlapping(
                    fragment_table.as_ptr() as *mut Tcpv4FragmentData,
        ...
        pub const fn as_ptr(&self) -> *mut Tcpv4ReceiveData {
            self.ptr.as_ptr()
        }

        pub const fn as_ref(&self) -> &Tcpv4ReceiveData {
    ...
    /// This is the same as [`Tcpv4TransmitData`], but with generically
    /// sized fragment table to allow for vectored writes.
    #[derive(Debug)]
    #[repr(C)]
    pub struct TransmitData<'a> {
        ptr: NonNull<Tcpv4TransmitData>,
        layout: Layout,
        _pd: PhantomData<FragmentData<'a>>,
    }
    ...
    impl<'a> TransmitData<'a> {
        pub fn new(data: &'a [&[u8]]) -> Self {
            ...
            let fragment_table: Vec<FragmentData> =
                data.iter().map(|d| FragmentData::new(d)).collect();
            let layout = {
                let header_layout = Layout::new::<Tcpv4TransmitData>();
                let payload_layout =
                    Layout::array::<FragmentData>(data.len()).expect("overflow not expected");
                let (layout, _) = header_layout
                    .extend(payload_layout)
            ...
            let ptr = unsafe {
                let ptr = alloc(layout).cast::<Tcpv4TransmitData>();
                let mut ptr = NonNull::new(ptr).expect("Allocation failed");
        ...
        pub const fn as_ptr(&self) -> *mut Tcpv4TransmitData {
            self.ptr.as_ptr()
        }
    }
    ...
    pub const fn make_completion_token(event: &Event) -> Tcpv4CompletionToken {
    ...
    pub const fn make_rx_token(event: &Event, rx_data: &ReceiveData) -> Tcpv4IoToken {
        let rx_data = rx_data.as_ptr();
        let packet = Tcpv4Packet { rx_data };
    ...
    pub const fn make_tx_token(event: &Event, tx_data: &TransmitData) -> Tcpv4IoToken {
        let tx_data = tx_data.as_ptr();
        let packet = Tcpv4Packet { tx_data };
        let completion_token = make_completion_token(event);
        Tcpv4IoToken {
            completion_token,
            packet,
...
mod wrappers {
    use core::net::Ipv4Addr;
    use uefi_raw::protocol::network::tcpv4::{Tcpv4AccessPoint, Tcpv4ConfigData, Tcpv4Option};
    ...
    impl From<AccessPoint> for Tcpv4AccessPoint {
    ...
    impl From<ConfigOptions> for Tcpv4Option {
