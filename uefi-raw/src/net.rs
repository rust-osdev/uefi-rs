...
impl IpAddress {
    ...
    ///
    /// # Safety
    /// Callers must be sure that the underlying address is an IPv6 address
    /// and that all bytes are initialized.
    ///
    /// # Panics
    /// Panics if is_ipv6 is false but extra bytes are present.
    #[must_use]
    pub unsafe fn to_std_ip_addr(self, is_ipv6: bool) -> StdIpAddr {
        if is_ipv6 {
...
mod tests {
    ...
    fn test_uefi_flow() {
        ...
        fn high_level_retrieve_ip(is_ipv6: bool) -> StdIpAddr {
            ...
            unsafe { efi_ip_addr.to_std_ip_addr(is_ipv6) }
        }
